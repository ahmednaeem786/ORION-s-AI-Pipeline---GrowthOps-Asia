...
class LLMEngine:
    ...
    def __init__(self, model_name: str = "gemini-3.6-flash"):
        ...
        if not api_key:
            raise RuntimeError("GEMINI_API_KEY is missing. Set it in .env or the process environment.")
    ...
    def extract_risks(self, text: str, company_name: str) -> list[DimensionRisk]:
        ...
        logger.info(f"Sending {len(text)} characters to {self.model} for {company_name}.")
        ...
        try:
            ...
            logger.info(f"Successfully extracted {len(result.extracted_risks)} risk dimensions.")
