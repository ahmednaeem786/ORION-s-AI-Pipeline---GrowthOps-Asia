...
class OrionPipeline:
    ...
    def process_submission(self, submission_file: str) -> ReviewerPayload:
        ...
        logger.info(
            f"Starting pipeline execution for submission file: {submission_file}"
        )
        ...
        auth_level = self.scorer.determine_authorization_level(
            composite_score, dimension_risks
        )
        ...
        logger.info(
            f"Pipeline finished for {submission_data.submission_id} in {elapsed}s with authorization verdict: '{auth_level}'."
        )
