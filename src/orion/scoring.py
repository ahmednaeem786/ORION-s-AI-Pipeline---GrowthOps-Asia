...
RISK_WEIGHTS = {"Low": 1.0, "Medium": 2.5, "High": 5.0}

# Dimension Weighting (inspired by BCBS & FATF)
# Regulatory and Security hold greater weightage in a digital infrastructure context, while Operational and Financial are slightly lower.
DIMENSION_WEIGHTS = {
...
# Zero-Tolerance dimensions that require immediate follow-up if flagged as "High" risk
# Bypasses the composite score and triggers direct escalation to human review.
CRITICAL_DOMAINS = ["Regulatory Integrity", "Data Security"]
...
class RiskScorer:
    ...
    def determine_authorization_level(
        composite_score: float, dimension_risks: list[DimensionRisk]
    ) -> str:
        """
        Maps composite score to recommended authorization tier using FATF and
        BCBS-inspired thresholds for oversight.
        Single failure in a critical domain (Regulatory or Security) will override the
        composite score and trigger a "Requires Supervisory Audit" recommendation.
        """
        for item in dimension_risks:
    ...
    def generate_follow_ups(dimension_risks: list[DimensionRisk]) -> list[str]:
        ...
        for item in dimension_risks:
            if item.risk_level == "High":
                if "regulatory" in item.dimension_name.lower():
                    ...
                    "security" in item.dimension_name.lower()
                    or "data" in item.dimension_name.lower()
                ):
                    questions.append(
        ...
        if not questions:
            questions.append(
                "No immediate high-risk follow-ups required. Standard periodic reporting applies."
            )
