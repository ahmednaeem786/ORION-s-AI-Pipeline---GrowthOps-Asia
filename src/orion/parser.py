...
class SectionNotFoundError(Exception):
    """Raised when the expected section (e.g., Item 1A) markers cannot be found in the document."""
...
class DocumentParser:
    ...
    def extract_item_1a(text: str) -> str:
        ...
        if not matches:
            error_msg = (
                "Automated parsing failed: Item 1A and 1B markers could not be located. "
                "Document format deviates from expected SEC 10-K standard."
            )
        ...
        logger.info(
            f"Successfully extracted {len(extracted_text)} characters of Risk Factors."
        )

        # Truncate to a safe token limit for standard LLMs
        # 1 token is roughly 4 chars, so 500,000 chars is roughly 125,000 tokens.
        max_chars = 500000
        if len(extracted_text) > max_chars:
            logger.info(
                f"Truncating extraction from {len(extracted_text)} to {max_chars} characters for token efficiency."
            )
    ...
    def process_document(cls, file_path: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"Loaded raw document {file_path} ({len(raw_text)} characters)."
            )
